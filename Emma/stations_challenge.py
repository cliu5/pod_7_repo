...
class SubwayStation(Station):
    ...
    def show_info(self):
        print(
            f'Subway lines {self.lines} stop at {self.station_name} Station at {self.location}')
        print(f' The following line stop at {self.station_name}')
        print(self.lines)
...
print(f'SubwayStation{self.station_name}  int:14th street, {self.location} 14th street and 7th avenue {self.lines}:['1', '2', '3', 'L'] ')
...
class BusStation():
    def __init__(self, busstation, station_name, routes):
        super().__init__(busstation, station_name)
        self.open_station = open_station
        self.close_station = close_station
        self.routes = routes
...
'''
BusStation = BusStation('NYC Megabus stop', location=('34th street and 12th avenue'), routes=['Boston', 'DC', 'Philly']
