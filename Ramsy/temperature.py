#Convert a temperature of 100 degrees fahrenheit to celsius
celsius_100 = (100 - 32 * (5/9))
print(celsius_100)
print('its a float because of the decimal point')

#Convert a temperature of 0 degrees fahrenheit to celsius
celsius_0 = (0 - 32 * (5/9))
print(celsius_0)

#Convert a temperature of 34.2 degrees fahrenheit to celsius
print(34.2 - 32 * (5/9))

#Convert a temperature of 5 degrees celsius to fahrenheit?
print(5 * (9/5) +32)

#Question: What is hotter, a temperature of 30.2 degrees celsius, or a temperature of 85.1 degrees fahrenheit?
print(30.2 * (9/5) +32)
print('30.2 celsius is 86.36 degrees fahrenheit therefore the former is hotter')